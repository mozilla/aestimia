...
exports.demo = function(req, res, next) {
  ...
  res.render('demo.html', {
    ...
    sections: parseRawDocSections()
  });
};

exports.docs = function(req, res) {
  return res.render('docs.html', {sections: parseRawDocSections()});
};

exports.history = function(req, res, next) {
  if (req.session.email) {
    Submission.findReviewed(req.session.email, function(err, submissions) {
      if (err) return next(err);
      submissions.reverse();
      res.render('history.html', {submissions: submissions});
    });
  } else
...
// developers.
var parseRawDocSections = exports.parseRawDocSections = function() {
  var fs = require('fs');
  var cheerio = require('cheerio');

  var docs = fs.readFileSync(__dirname + '/../views/docs-raw.html',
                             'utf8');
